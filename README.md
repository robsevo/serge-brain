...
The doctor reads your system — `pacman`, `apt`, `brew`, `dnf`, WSL — and prints
the exact install command for anything missing, rather than naming a tool and
leaving you to find it. Then it checks each key **against the real provider**, so
a typo surfaces there instead of later as a routing error that looks like a Serge
bug. It exits `Ready.` as soon as one provider key works.

You need one key, not all of them. Gemini is the easiest — free, no card, and it
backs both seats Serge uses by default, so a single key is a complete install.
Free tiers run out, so any provider also takes a spare account: add the same name
with `_2` and run `rotate-keys.sh`. That is config, not new routing — LiteLLM
already runs four deployments behind `free-qwen` today.
